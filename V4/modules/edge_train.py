...
class EdgeModel(torch.nn.Module, ABC):
    def __init__(self, args):
        ...
        for x in range(1,6):
            self.sub_round = x
            user_graph_path = self.args.path + f'userlist_group_{x}.csv'
            ...
            user_embedding_path = self.args.path+f'sub/updated_embeddings_{x}.pk'
            self.user_embeds = get_user_embedding(args, user_embedding_path)
            self.item_embeds = get_item_embedding(args)
    ...
    def forward(self, inputs, train=True):
        ...
        Index = torch.arange(self.user_embeds[0]).cuda()
        # 将其传递给用户/服务嵌入层self.user/serv_embeds以获取所有用户/服务的嵌入向量
        user_embeds = self.user_embeds(Index)
        Index = torch.arange(self.item_embeds[0]).cuda()
        item_embeds = self.item_embeds(Index)
        ...
        self.agg_user_embeds.append(user_embeds)
        self.agg_item_embeds.append(item_embeds)

        agg_user_embeds, agg_item_embeds = self.agg_user_embeds,self.agg_item_embeds
        #最后一个云边服务器训练完之后回传
        if self.sub_round == 5:
            agg_user_embeds, agg_item_embeds = self.get_final_embedding(agg_user_embeds,agg_item_embeds)
            return agg_user_embeds, agg_item_embeds
        return estimated,agg_user_embeds,agg_item_embeds
