...
from datasets.RecEraser import *
...
# ## 合并异常值数据矩阵
# # 将输入的数据矩阵（Tensor）与一个异常值标记矩阵（outlier）合并，通过将特定的数据点标记为异常（值为0），从而对原数据矩阵进行修改
# def merge_Tensor_outlier(Tensor, outlier):
#     merge = Tensor
#     for i in range(len(Tensor)):
#         for j in range(len(Tensor[0])):
#             if Tensor[i][j] >= 0 and outlier[i][j] == 1: # 确认当前位置的值是非负的并检查对应位置在outlier矩阵中是否被标记为异常
#                 merge[i][j] = 0
#
#     return merge
...
class ShardedTensorDataset(Dataset):
    ...
    def __init__(self, data, First, args):
        ...
        if First:
            ...
            # node2vec_Earser
            flag = True
            self.split_train_Tensor = user_based_balanced_parition(self.train_Tensor, args)
            # self.split_train_Tensor = interaction_based_balanced_parition(self.train_Tensor, args)
            log('\tReceraser 基于用户切割完毕')
