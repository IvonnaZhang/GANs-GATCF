...
class MetaModel(torch.nn.Module, ABC):
    ...
    #         print(inputs)
    #         # print("input before", len(inputs[0]), "value before", value.shape)
    ...
    #         print("Shape mismatch:", "Pred shape:", len(inputs), "Value shape:", value.shape)
    #         # inputs = inputs.to(self.args.device)
    #         # value = value.to(self.args.device)
    #
    #         pred = self.forward(inputs, False)
    #         if pred.shape != value.shape:
    #             print("Shape mismatch:", "Pred shape:", pred.shape, "Value shape:", value.shape)
    #             continue
    #
    ...
    #
    #     # 确保数据类型一致，适用于后续计算
    #     preds = preds.float()  # 可能需要根据您的实际情况调整数据类型
    #     reals = reals.float()
    #
    ...
    def test_one_epoch(self, dataModule):
        ...
        for test_Batch in tqdm(dataModule.test_loader, disable=not self.args.program_test):
            ...
            if self.args.device != 'cpu':
                inputs, value = to_cuda(inputs, value, self.args)
            pred = self.forward(inputs, False)
            # print("input after", pred.shape, "value after", value.shape)
            preds[writeIdx:writeIdx + len(pred)] = pred
            reals[writeIdx:writeIdx + len(value)] = value
            writeIdx += len(pred)
